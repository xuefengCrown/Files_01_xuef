...
# It currently looks up names in the current environment, returns self-evaluating
# expressions (like numbers) and evaluates special forms
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        try:
            ...
            raise SchemeError('not valid operator')
        # 2. Evaluate all of the operands
        rest_v = rest.map(lambda x:scheme_eval(x, env))
        # 3. Apply the procedure on the evaluated operands
        return scheme_apply(proc, rest_v, env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    # scm> (begin (+ 2 3) (+ 5 6))
    # Pair(Pair('+', Pair(2, Pair(3, nil))), Pair(Pair('+', Pair(5, Pair(6, nil))), nil))
    if expressions == nil:
        return None
...
class Frame:
    ...
    def __init__(self, parent):
        ...
        #bindings is a dictionary representing the bindings in the frame.
        #It maps Scheme symbols (represented as Python strings) to Scheme values.
        self.bindings = {}
    ...
    # 这里我们可以明白 define 在底层，在计算机内部 都做了些什么
    ...
    # create new call frames for user-defined procedures. 
    def make_child_frame(self, formals, vals):
        ...
        vals = vals.map(lambda x:scheme_eval(x, self))
        formals_rest, vals_rest = formals, vals
        while formals_rest != nil:
            child_frame.bindings[formals_rest.first] = vals_rest.first
            formals_rest, vals_rest = formals_rest.second, vals_rest.second
        #print(child_frame.bindings)
        return child_frame
        # END PROBLEM 11
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            apply_rs = self.fn(*python_args)
        except TypeError:
            raise SchemeError('func args type error')
        return apply_rs
        # END PROBLEM 4
...
class LambdaProcedure(Procedure):
    ...
    """
        create a new Frame instance using the make_child_frame method of the appropriate parent frame,
        binding formal parameters to argument values.
        Since lambdas are lexically scoped, your new frame should be a child
        of the frame in which the lambda is defined.

        The env provided as an argument to make_call_frame is instead(而是) the frame in which
        the procedure is called, which will be useful when you implement dynamically scoped
        procedures in problem 16.
    """
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        #print(repr(rest))
        # rest.first 清除第一层括号???
        rest_v = scheme_eval(rest.first, env) # ???
        env.define(target, rest_v)
        return target
        # END PROBLEM 6
        ...
        "*** YOUR CODE HERE ***"
        """
        we'd like to be able to use the shorthand form of defining named procedures:
            scm> (define (f x) (* x 2))
            f
        Modify the do_define_form function so that it correctly handles the
        shorthand procedure definition form above.
        ...
        proc = LambdaProcedure(target.second, expressions.second, env)
        env.define(proc_name, proc)
        return proc_name
        # END PROBLEM 10
    else:
...
"""
In Scheme, you can quote expressions in two ways:
with the quote special form or with the symbol '
    scm> (quote hello)
    hello
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    # Implement the do_quote_form function so that
...
"""
    ...
    formals, body, env
"""
# which creates a LambdaProcedure instance
def do_lambda_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    #print(repr(expressions))
    proc = LambdaProcedure(expressions.first, expressions.second, env)
    return proc
    # END PROBLEM 9
...
def do_and_form(expressions, env):
    ...
    rs = scheme_eval(expressions.first, env)
    rest = expressions.second
    while rest != nil and scheme_truep(rs):
        rs = scheme_eval(rest.first, env)
        rest = rest.second
    if rest == nil: return rs
    return False
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    rs = scheme_eval(expressions.first, env)
    rest = expressions.second
    while rest != nil and scheme_falsep(rs):
        rs = scheme_eval(rest.first, env)
        rest = rest.second
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            "*** YOUR CODE HERE ***"
            #print(repr(clause), repr(clause.first))
            #print(test)
...
Implement make_let_frame, which returns a child frame of env that binds the symbol in each
element of bindings to the value of its corresponding expression.
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
##    print(repr(bindings))
##    print(check_form(bindings))
    
    formals = bindings.map(lambda x: x.first)
    vals = bindings.map(lambda x: scheme_eval(x.second.first, env))
...
#################
"""
All of the Scheme procedures we've seen so far use
lexical scoping: the parent of the new call frame is the environment in which the procedure was defined.
Another type of scoping, which is not standard in Scheme, is called
dynamic scoping: the parent of the new call frame is the environment in which the procedure was evaluated.
With dynamic scoping, calling the same procedure in different parts of your code can lead to
different results (because of varying parent frames).
...
class MuProcedure(Procedure):
    ...
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS"""
        "*** YOUR CODE HERE ***"
        ### 这里的 env 有什么作用??????
        print('make_call_frame of mu', env.bindings, self.body)
...
def do_mu_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    #print(repr(expressions))
    args = expressions.first
    proc_body = expressions.second
    #print(args, proc_body)
    proc = MuProcedure(args, proc_body)
    # call_frame = proc.make_call_frame(args, env)
    return proc
    # END PROBLEM 16
