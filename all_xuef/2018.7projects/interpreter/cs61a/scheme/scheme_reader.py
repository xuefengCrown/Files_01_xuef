...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        """
        if val == "'":
            # 'bagel should be represented as Pair('quote', Pair('bagel', nil))
            wrapped = Pair('quote', Pair(scheme_read(src), nil))
            #print(repr(wrapped), wrapped)
            return wrapped
        elif val == "`":
...
## added by xuef
def xuef_test():
    rs = scheme_read(Buffer(tokenize_lines(['(+ 1 2)'])))
    print(repr(rs))
    rs2 = read_tail(Buffer(tokenize_lines(['2 3)'])))
    print(repr(rs2))
if __name__ == '__main__':
##    xuef_test()
##    rs = read_line('(1 (2 3) (4 (5)))')
##    rs = read_line('(1 (9 8) . 7)')
    rs = read_line('(hi there . (cs . (student)))')
    print(repr(rs))
