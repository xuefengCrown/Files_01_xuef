...
(define (negate-bool bool)
  (if (ask bool 'val) *PY-FALSE* *PY-TRUE*))
...
(define-class (py-list val)
  ...
  (method (__setitem__ index item)
    ...
    (let ((n (ask index 'val)))
      ...
      (if (< n 0) (set! n (+ n (length val))))
      (replace-item n val)))
  (method (__contains__ other) ; Person A, Question 4
    (define (helper sc-list)
      (if (null? sc-list)
	  *PY-FALSE*
	  (if (ask (py-apply (ask (car sc-list) '__eq__) (list other)) 'true?)
	      *PY-TRUE*
	      (helper (cdr sc-list)))))
    (let ((con-list val))
      (helper con-list)))
