...
(define (py-read)
  (define (get-indent-and-tokens)
     (define (helper-indentation num) ;; COMMON EXERCISES #2
       (let ((x (peek-char)))
	       (if (not (eq? x #\space))
	           (cons num (get-tokens '()))
             (begin (read-char) (helper-indentation (+ num 1))))))
   (helper-indentation 0))
  ...
  (define (get-num num-so-far) ;; Person B, Question 3
    (define (get-num-helper nsf boolean-flag) 
      (let ((p (peek-char)))
        (cond ((and (eq? p #\.) boolean-flag) nsf)
	            ((and (eq? p #\.) (eq? boolean-flag #f)) (get-num-helper (word nsf (char->symbol (read-char))) #t))
	            ((char-numeric? p) (get-num-helper (word nsf (char->symbol (read-char))) boolean-flag))
	            (else nsf))))
   (get-num-helper num-so-far #f))
      ...
      (cond ((eq? char #\+) (if (eq? next #\=) (begin (read-char) '+=) '+))
	    ...
	    ((eq? char #\*)
	     (cond ((eq? next #\*)
		   ...
		   ((eq? next #\=) (read-char) '*=)
		   (else '*))))))
  (define (get-string type) ; Person A, Question 3
    ...
    (define (helper so-far)
      (let ((next (peek-char)))
	(if (eq? next type)
	    (begin (read-char) so-far)
	    (helper (append so-far (list (read-char)))))))
    (let ((result (helper (list (read-char)))))
      result))
  (define (ignore-comment) ;; COMMON EXERCISES #1
     (define (helper x)
       (if (or (eof-object? x) (eq? x #\newline))
