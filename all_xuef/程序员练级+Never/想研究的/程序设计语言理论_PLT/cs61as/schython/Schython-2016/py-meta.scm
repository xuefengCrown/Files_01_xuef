...
;;;;;;
;;;;;; PERSON A: William Park (cs61as-cb)
;;;;;; PERSON B: Jorge Paulo Campos (cs61as-ao)
;;;;;;
  ...
  (if (ask line-obj 'empty?)
	...
	(cond ((infix-op? token) ;; arithmetic infix operators
		 ...
		 (handle-infix (py-apply (ask val (lookup-op token))
	      ...
	      ((and? token) ; Person A, Question 5
	       (if (ask val 'true?)
		   (py-eval line-obj env)
		   (begin (eat-tokens line-obj)
			  *PY-FALSE*)))
	      ((or? token) ; Person A, Question 5
	       (if (ask val 'true?)
		   (begin (eat-tokens line-obj)
			  val)
		   (py-eval line-obj env)))
	      ;; test for membership
	      ((in? token) ; Person B, Question 5
	       (let ((right-operand (eval-item line-obj env)))
		       (ask right-operand '__contains__ val)))
	      ((not? token) ; Person B, Question 5
	       (let ((p (ask line-obj 'peek)))
		       (if (eq? p 'in)
...
(define (collect-key-value line-obj env close-token) ;COMMON EXERCISE #8
  (define (ckv-helper so-far close-tok)
    (if (ask line-obj 'empty?)
	      so-far
	      (let ((p (ask line-obj 'peek)) (placeholder1 '()) (placeholder2 '()))
          (if (eq? p close-token) 
	            so-far
              (begin (set! placeholder1 (py-eval (make-line-obj (cons 0 (tokens-before-colon line-obj close-tok))) env))
                     (ask line-obj 'next) ; eat the colon
	                   (set! placeholder2 (py-eval (make-line-obj (cons 0 (tokens-after-colon line-obj close-tok))) env))
	                   (ask line-obj'next) ; eat the comma or the close-token
	                   (if (null? so-far)
	                       (set! so-far (list (cons placeholder1 placeholder2))) 
	                   (set! so-far (append so-far (list (cons placeholder1 placeholder2))))) ;put dictionary in proper order
                     (set! placeholder1 '())
                     (set! placeholder2 '())
                     (ckv-helper so-far close-token))))))
      (ckv-helper '() close-token))

(define (tokens-before-colon line-obj ct)
  (define (tbc-helper so-far ct)
    (let ((p (ask line-obj 'peek)))
      (if (eq? p ':)
	  so-far
          (begin (let ((n (ask line-obj 'next)))
          (set! so-far (append so-far (list n)))
          (tbc-helper so-far ct))))))
  (tbc-helper '() ct))

(define (tokens-after-colon line-obj ct)
  (define (tac-helper so-far ct)
    (let ((p (ask line-obj 'peek)))
      (if (or (comma? p) (eq? p ct))
          so-far
          (begin (let ((n (ask line-obj 'next)))
          (set! so-far (append so-far (list n)))
          (tac-helper so-far ct))))))
   (tac-helper '() ct))
...
;; Conditionals
(define (make-if-block line-obj env) ; Person B, Question 7
  (let ((body (read-block (ask line-obj 'indentation) env))
	      (pred (if-pred-helper line-obj)))
     (list '*BLOCK* '*IF-BLOCK* pred (split-block body))))

(define (if-pred-helper line-obj)
  (let ((first-line-indent (ask line-obj 'indentation)))
    (define (if-helper2 so-far)
	    (if (ask line-obj 'empty?)
	        (cons first-line-indent so-far)
          (let ((n (ask line-obj 'next)))
            (if (eq? n '#\:)
	              (cons first-line-indent so-far)
	              (if-helper2 (append so-far (list n)))))))
  (if-helper2 '())))

(define (if-block-pred block) ; Person B, Question 7
  (caddr block))
(define (if-block-body block) ; Person B, Question 7
  (car (cadddr block)))
(define (if-block-else block) ; Person B, Question 7
  (cdr (cadddr block)))
      

(define (eval-if-block block env) ; Person B, Question 7
...
(define (make-elif-block line-obj env) ; Person B, Question 7
  (let ((body (read-block (ask line-obj 'indentation) env))
	      (pred (if-pred-helper line-obj)))
    (list '*BLOCK* '*ELIF-BLOCK* pred (split-block body))))

(define (elif-block-pred block) (caddr block)) ; Person B, Question 7
(define (elif-block-body block) (car (cadddr block)))
(define (elif-block-else block) (cdr (cadddr block)))

(define (eval-elif-block block env) ; Person B, Question 7
  (let ((elif-pred (elif-block-pred block))
	      (elif-body (elif-block-body block))
	      (elif-else-clause (elif-block-else block)))
    (define (elif->if block env)
      (eval-if-block (list '*BLOCK* '*IF-BLOCK* elif-pred (cons elif-body elif-else-clause)) env))
  (elif->if block env)))
...
;; While loops
(define (make-while-block line-obj env) ;COMMON EXERCISES #6
    (define (form-pred so-far)
      (let ((n (ask line-obj 'next)))
        (if (eq? n ':)
            so-far
            (form-pred (append so-far (list n))))))
    (let ((body (read-block (ask line-obj 'indentation) env))
          (pred (cons (ask line-obj 'indentation) (form-pred '()))))
      (list '*BLOCK* '*WHILE-BLOCK* (split-block body) pred)))
...
;; For loops
(define (make-for-block line-obj env) ; Person A, Question 7
  (let ((var (ask line-obj 'next)))
    (define (make-collection so-far)
      (let ((token (ask line-obj 'next)))
        (cond ((or (eq? token var) (eq? token 'in)) (make-collection so-far))
              ((eq? token ':) so-far)
              (else
               (make-collection (append so-far (list token)))))))
    (let ((collection (make-collection '()))
          (body (read-block (ask line-obj 'indentation) env)))
      (list '*BLOCK* '*FOR-BLOCK* (cons var collection) (split-block body)))))
...
(define (eval-for-block block env)
  (let ((collection (for-block-collection block)))
    (let ((line-obj (make-line-obj (cons '*DUMMY-INDENT* collection))))
      (let ((py-list (py-eval line-obj env)))
        (let ((result (ask py-list '__iter__ (for-block-var block) (for-block-body block) env)))
          (cond ((eq? result '*BREAK*) *NONE*)
                ((for-block-else block) (eval-item (make-line-obj (for-block-else block)) env))
        (else *NONE*)))))))
