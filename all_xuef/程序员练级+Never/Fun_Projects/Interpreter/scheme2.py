...
def analyze(toks):      #语法分析:-->表达式树
    ...
    if tok == "'": # 提供对 ' 的支持
        return ["'", analyze(toks)]
    elif type(tok) in (int,) or tok not in ('(',')'):
        return tok
    elif toks[0] == '(':
...
context.update(BUILTINS) #将内置函数包加入到全局环境
...
def scheme_eval(exp_tree, env=env0):
    if is_number(exp_tree):                         # 数字
        ...
        _,test,true_branch,false_branch = exp_tree
        if scheme_eval(test, env) == True:
            return scheme_eval(true_branch, env)
        else:
            return scheme_eval(false_branch, env)
...
if __name__ == '__main__':
    ...
    #print(analyze(tokenize("(if (eq? 2 2) (+ 4 4) (* 2 2))")))
"""
