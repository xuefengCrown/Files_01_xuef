...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#pageNoForTurn").change(function(){
			...
			var href = "BookServlet?method=getBooks" + href
			//alert(href)
			window.location.href = href
		})
	})
	...
	<center>
		...
		<table>
			<c:forEach items="${allBooks.list}" var="book">
				<tr>
					<td><a href="BookServlet?method=getBookDetail&pageNo=${allBooks.pageNo}
					&id=${book.id}">
					${ book.title }</a>--
					${book.author}
					</td>
					<td>${ book.price }</td>
					<td> <a href="BookServlet?method=add2Cart&pageNo=${allBooks.pageNo}
		...
		<c:if test="${allBooks.hasPre}">
			<a href="BookServlet?method=getBooks&pageNo=1">首页</a>
			 &nbsp;
			<a href="BookServlet?method=getBooks&pageNo=${allBooks.prePage}">上一页</a>
		</c:if>
		
		<c:if test="${allBooks.hasNext}">
			<a href="BookServlet?method=getBooks&pageNo=${allBooks.nextPage}">下一页</a>
			 &nbsp;
			<a href="BookServlet?method=getBooks&pageNo=${allBooks.totalPageNumber}">末页</a>
		</c:if>
		...
		<c:if test="${!empty sessionScope.shopCart}">
			...
			您的购物车中有 ${sessionScope.shopCart.totalNum} 本书 <a href="BookServlet?method=lookCart
