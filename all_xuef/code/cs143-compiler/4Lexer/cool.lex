...
/*  Stuff enclosed in %{ %} is copied verbatim逐字的 to the lexer class
 ...
 *  work.  */
	//if(eofReached) return new Symbol(TokenConstants.EOF);
    switch(yy_lexical_state) {
        ...
        */
		case STRING:
		  if(eofReached) break;
		  eofReached = true;
		  if(escapeFounded) return new Symbol(TokenConstants.ERROR, "String contains escaped null character.");
...
<BLOCK_COMMENT> "*"   { }
<BLOCK_COMMENT> ")"   { }
<BLOCK_COMMENT> "(*"  { nestedCommentDepth++; }
<BLOCK_COMMENT> "*)" {
...
<BLOCK_COMMENT> \n    { curr_lineno++; }
<BLOCK_COMMENT> [^*\n\(\)]+  { /* Do Nothing--ignore all comment contents */ }
...
<STRING> \" { // If an escape character appears before a quote, we must put a quote into the string
  ...
  if(string_buf.length()>0 && string_buf.charAt(string_buf.length()-1)=='\\' && escapeFounded) {
...
<STRING> \n { // Code for newline characters
  ...
  if(!escapeFounded || string_buf.length()==0) {
    curr_lineno++;
    yybegin(YYINITIAL);
...
<STRING> [^\n\"] { 
  ...
  if(stringTooLong || nullInString) {
    ...
    if(string_buf.charAt(0)=='\\') escapeFounded = true;
  } else {
    ...
    if(yytext().charAt(0)=='\0' || nullInString) {
      ...
      if(string_buf.charAt(string_buf.length()-1)=='\\' && escapeFounded) {
...
<YYINITIAL> [^\n] { return new Symbol(TokenConstants.ERROR, new String(yytext())); } /* for what??? */
