...
def canPartition(nums):
    ...
    def canPartTwo(lst, sum1, sum2):
        ...
        first = lst[0]
        # 如果没有备忘，再计算
        if (tuple(lst), sum1, sum2) not in dp:
            #print(dp)
            sub1 = canPartTwo(lst[1:], sum1-first, sum2)
            sub2 = canPartTwo(lst[1:], sum1, sum2-first)
            dp[(tuple(lst), sum1, sum2)] = sub1 or sub2
##            print(lst[1:], "--", sum1-first, "**", sum2)
##            print(lst[1:], "--", sum1, "**", sum2-first)
##            print(lst, "--", sum1, "**", sum2)
            
        #print(lst, "--", sum1, "--", sum2, "**>", dp[(tuple(lst), sum1, sum2)])
        return dp[(tuple(lst), sum1, sum2)]
