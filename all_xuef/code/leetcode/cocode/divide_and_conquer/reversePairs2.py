...
def mergesort_and_count(lst, start, end):
    ...
    i, k, t = start, 0, start
    merge_res = [0]*(end-start+1)
    for j in range(mid+1, end+1):
        i = bisect_left(lst[start:mid+1], lst[j]*2 + 1)
        if i <= mid-start and lst[i+start] > lst[j]*2: # 利用了有序性
            count += mid-(i+start)+1
        # t 归并用左指针
        # [1,3] [2]    
        while t <= mid and lst[t] < lst[j]:
            merge_res[k] = lst[t]
            k += 1
            t += 1
        merge_res[k] = lst[j]
        k += 1
    merge_res[k:mid+1-t+k] = lst[t:mid+1]
    #print(merge_res, start, end)
    # 至此, count是lst[start-end]的所有reverse pairs的数目
    #print(start, "-", end, "all recerse pairs: ", count)
    # 原地归并 lst[start-mid] lst[mid+1-end]
    #merge(lst, start, mid, end)
    #for i in range(start, end+1):
    #    lst[i] = merge_res[i-start]
    lst[start:end+1] = merge_res[:end+1-start]
    return count
