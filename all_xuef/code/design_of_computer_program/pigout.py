...
def hold(state):
    ...
    # new_state = (state[0]+1)%2,state[2],state[1]+state[3],0
    # return new_state
...
def roll(state, d):
    ...
    # your code here
    if d==1:
        new_state = (state[0]+1)%2,state[2],state[1]+1,0
    else:
        new_state = state[0],state[1],state[2],state[3]+d
    return new_state

# mapping from player to other playerv
other = {0:1, 1:0}    
def test():    
