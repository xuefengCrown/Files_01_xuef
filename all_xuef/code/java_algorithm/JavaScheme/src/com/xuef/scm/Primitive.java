...
/**
 * A primitive is a procedure that is defined as part of the Scheme report, and
 * is implemented in Java code.
 * 
...
public class Primitive extends Procedure {
	...
	// 参数个数 上界
	int maxArgs;
	// idNumber 唯一标记一个 primitive procedure
	int idNumber;
	...
	private static final int EQ = 0, LT = 1, GT = 2, GE = 3, LE = 4, ABS = 5,
			...
			NEGATIVEQ = 122, CHARCMP = 123 /* to 127 */, CHARCICMP = 128 /* to 132 */,
	...
	public static Frame installPrimitives(Frame env) {

		int n = Integer.MAX_VALUE;
		// scheme primitive procedure --> java method
		env.defPrim("*", TIMES, 0, n)
				.defPrim("+", PLUS, 0, n).defPrim("-", MINUS, 1, n)
				.defPrim("/", DIVIDE, 1, n).defPrim("<", LT, 2, n)
				.defPrim("<=", LE, 2, n).defPrim("=", EQ, 2, n)
				.defPrim(">", GT, 2, n).defPrim(">=", GE, 2, n)
				.defPrim("abs", ABS, 1).defPrim("acos", ACOS, 1)
				.defPrim("append", APPEND, 0, n).defPrim("apply", APPLY, 2, n)
				.defPrim("asin", ASIN, 1).defPrim("assoc", ASSOC, 2)
				.defPrim("assq", ASSQ, 2).defPrim("assv", ASSV, 2)
				.defPrim("atan", ATAN, 1).defPrim("boolean?", BOOLEANQ, 1)
				.defPrim("caaaar", CXR, 1).defPrim("caaadr", CXR, 1)
				.defPrim("caaar", CXR, 1).defPrim("caadar", CXR, 1)
				.defPrim("caaddr", CXR, 1).defPrim("caadr", CXR, 1)
				.defPrim("caar", CXR, 1).defPrim("cadaar", CXR, 1)
				.defPrim("cadadr", CXR, 1).defPrim("cadar", CXR, 1)
				.defPrim("caddar", CXR, 1).defPrim("cadddr", CXR, 1)
				.defPrim("caddr", THIRD, 1).defPrim("cadr", SECOND, 1)
				...
				.defPrim("car", CAR, 1).defPrim("cdaaar", CXR, 1)
				.defPrim("cdaadr", CXR, 1).defPrim("cdaar", CXR, 1)
				.defPrim("cdadar", CXR, 1).defPrim("cdaddr", CXR, 1)
				.defPrim("cdadr", CXR, 1).defPrim("cdar", CXR, 1)
				.defPrim("cddaar", CXR, 1).defPrim("cddadr", CXR, 1)
				.defPrim("cddar", CXR, 1).defPrim("cdddar", CXR, 1)
				.defPrim("cddddr", CXR, 1).defPrim("cdddr", CXR, 1)
				.defPrim("cddr", CXR, 1).defPrim("cdr", CDR, 1)
				...
				.defPrim("char>?", CHARCMP + GT, 2).defPrim("char?", CHARQ, 1)
				.defPrim("close-input-port", CLOSEINPUTPORT, 1)
				.defPrim("close-output-port", CLOSEOUTPUTPORT, 1)
				.defPrim("complex?", NUMBERQ, 1).defPrim("cons", CONS, 2)
				.defPrim("cos", COS, 1)
				.defPrim("current-input-port", CURRENTINPUTPORT, 0)
				.defPrim("current-output-port", CURRENTOUTPUTPORT, 0)
				.defPrim("display", DISPLAY, 1, 2)
				.defPrim("eof-object?", EOFOBJECTQ, 1).defPrim("eq?", EQQ, 2)
				.defPrim("equal?", EQUALQ, 2).defPrim("eqv?", EQVQ, 2)
				.defPrim("eval", EVAL, 1, 2).defPrim("even?", EVENQ, 1)
				.defPrim("exact?", INTEGERQ, 1).defPrim("exp", EXP, 1)
				.defPrim("expt", EXPT, 2).defPrim("force", FORCE, 1)
				.defPrim("for-each", FOREACH, 1, n).defPrim("gcd", GCD, 0, n)
				.defPrim("inexact?", INEXACTQ, 1)
				.defPrim("input-port?", INPUTPORTQ, 1)
				.defPrim("integer->char", INTEGERTOCHAR, 1)
				.defPrim("integer?", INTEGERQ, 1).defPrim("lcm", LCM, 0, n)
				.defPrim("length", LENGTH, 1).defPrim("list", LIST, 0, n)
				.defPrim("list->string", LISTTOSTRING, 1)
				.defPrim("list->vector", LISTTOVECTOR, 1)
				.defPrim("list-ref", LISTREF, 2)
				.defPrim("list-tail", LISTTAIL, 2).defPrim("list?", LISTQ, 1)
				.defPrim("load", LOAD, 1).defPrim("log", LOG, 1)
				.defPrim("macro-expand", MACROEXPAND, 1)
				.defPrim("make-string", MAKESTRING, 1, 2)
				.defPrim("make-vector", MAKEVECTOR, 1, 2)
				.defPrim("map", MAP, 1, n).defPrim("max", MAX, 1, n)
				.defPrim("member", MEMBER, 2).defPrim("memq", MEMQ, 2)
				.defPrim("memv", MEMV, 2).defPrim("min", MIN, 1, n)
				.defPrim("modulo", MODULO, 2)
				.defPrim("negative?", NEGATIVEQ, 1)
				.defPrim("newline", NEWLINE, 0, 1).defPrim("not", NOT, 1)
				.defPrim("null?", NULLQ, 1)
				.defPrim("number->string", NUMBERTOSTRING, 1, 2)
				.defPrim("number?", NUMBERQ, 1).defPrim("odd?", ODDQ, 1)
				...
				.defPrim("rational?", INTEGERQ, 1).defPrim("read", READ, 0, 1)
				.defPrim("read-char", READCHAR, 0, 1)
				.defPrim("real?", NUMBERQ, 1)
				.defPrim("remainder", REMAINDER, 2)
				.defPrim("reverse", REVERSE, 1).defPrim("round", ROUND, 1)
				.defPrim("set-car!", SETCAR, 2).defPrim("set-cdr!", SETCDR, 2)
				.defPrim("sin", SIN, 1)
				.defPrim("sqrt", SQRT, 1)
				...
				.defPrim("new", NEW, 1).defPrim("class", CLASS, 1)
				.defPrim("method", METHOD, 2, n).defPrim("exit", EXIT, 0, 1)
	...
	public Object apply(Scheme interpreter, Object args) {
		...
		// 由此可见，Scheme中只有 #f 为假，其余皆为真。
		// In Scheme, only False is a false value.
		/* Not returns #t if obj is false, and returns #f otherwise. 
		 * All other values (including 0 and nil) are true values.
		 * 
		...
		case FORCE:
			return (!(x instanceof Procedure)) ? x : proc(x)
					.apply(interpreter, null);
		case MACROEXPAND:
			//return Macro.macroExpand(interpreter, x);
		case PROCEDUREQ:
			return truth(x instanceof Procedure);
		case APPLY:
			return proc(x).apply(interpreter, listStar(rest(args)));
		case MAP:
			return map(proc(x), rest(args), interpreter, list(null));
		case FOREACH:
			return map(proc(x), rest(args), interpreter, null);
		case CALLCC:
			/*
		...
		case CALLWITHOUTPUTFILE:
			...
			try {
				p = openOutputFile(x);
				z = proc(y).apply(interpreter, list(p));
			} finally {
				if (p != null)
					p.close();
			}
		...
		case CALLWITHINPUTFILE:
			...
			try {
				p2 = openInputFile(x);
				z = proc(y).apply(interpreter, list(p2));
			} finally {
				if (p2 != null)
					p2.close();
			}
		...
		case TIMECALL:
			...
			for (int i = 0; i < nTimes; i++) {
				ans = proc(x).apply(interpreter, null);
			}
			long time = System.currentTimeMillis() - startTime;
			long mem = startMem - runtime.freeMemory();
			return cons(ans,
					list(list(num(time), "msec"), list(num(mem), "bytes")));
		default:
	...
	/**
	 * Return <0 if x is alphabetically first, >0 if y is first, 0 if same. Case
	 * insensitive iff ci is true. Error if not both chars.
	 **/
	...
	/**
	 * Return <0 if x is alphabetically first, >0 if y is first, 0 if same. Case
	 * insensitive iff ci is true. Error if not strings.
	 **/
	...
	/**
	 * Map proc over a list of lists of args, in the given interpreter. If
	 * result is non-null, accumulate the results of each call there and return
	 * that at the end. Otherwise, just return null.
	 **/
	static Pair map(Procedure proc, Object args, Scheme interpreter, Pair result) {
		Pair accum = result;
		if (rest(args) == null) {
			args = first(args);
			while (args instanceof Pair) {
				Object x = proc.apply(interpreter, list(first(args)));
				if (accum != null)
					accum = (Pair) (accum.rest = list(x));
				args = rest(args);
			}
		} else {
			Procedure car = proc(interpreter.eval("car")), 
					cdr = proc(interpreter.eval("cdr"));
			while (first(args) instanceof Pair) {
				Object x = proc.apply(interpreter,
						map(car, list(args), interpreter, list(null)));
				if (accum != null)
					accum = (Pair) (accum.rest = list(x));
				args = map(cdr, list(args), interpreter, list(null));
			}
		}
