...
public class JavaMethod extends Procedure {
	...
	public Object apply(Scheme interpreter, Object args) {
		try {
			if (isStatic)
				...
				return method.invoke(first(args), toArray(rest(args)));
		...
		return error("Bad Java Method application:" + this + stringify(args)
				+ ", ");
	}
	...
	/**
	 * Convert a list of Objects into an array. Peek at the argClasses array to
	 * see what's expected. That enables us to convert between Double and
	 * Integer, something Java won't do automatically.
	 **/
