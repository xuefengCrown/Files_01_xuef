...
final class HttpProcessor implements Lifecycle, Runnable {
    ...
    synchronized void assign(Socket socket) {
    	System.out.println("assign 所在线程名：" + Thread.currentThread().getName());
        // Wait for the Processor to get the previous Socket
        while (available) { // HttpProcessor线程刚启动时，available is false
            try {
            	System.out.println("assign() is waiting...");
                wait();
            } catch (InterruptedException e) {
            }
        }
        
        System.out.println("assign 继续执行...");
        try {
			Thread.sleep(10000);
		} catch (InterruptedException e) {
			e.printStackTrace();
    ...
    private synchronized Socket await() {
        ...
        while (!available) {
            try {
            	System.out.println("await is Waiting for the Connector to provide a new Socket");
                wait();
            } catch (InterruptedException e) {
            }
        }
        System.out.println("await() 继续执行...");
    ...
    public void run() {
        ...
        while (!stopped) {
        	System.out.println("HttpProcessor thread is running...");
            // Wait for the next socket to be assigned
        	// 直到assign()被调用前，await()会一直阻塞
            Socket socket = await();
            System.out.println("HttpProcessor 获得 socket...");
            if (socket == null)
                continue;
